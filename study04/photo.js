...
for(var i=0; i<todayPhoto.length; i++){
    ...
    imgs.push('<div class="news">' + '<img clas="img" src="' + todayPhoto[i].img + '">' + '<span class="title">' + todayPhoto[i].title + '</span>' + '</div>');
	
//	imgs.push( '<img src="' + todayPhoto[i].img + '"> ');
//	titles.push('<div class="title">' + todayPhoto[i].title + '</div>');
	

//	str += '<img src="' + todayPhoto[i].img + '"> ';
//	str += "<div id='strs'>" + todayPhoto[i].title + "</div>";
}
...
//wrap.innerHTML = str;
//wrap.innerHTML = imgs.join("");
wrap.innerHTML = "<div class='size'> <h1>오늘의 사진</h1><hr> <div  id='d1'>" + img1String + "</div> "
					+ "<div id='d2' class='hide'>" + img2String + "</div></div>";
